...
// CORS correcto para extensiones Chrome con credentials
$origin = $_SERVER['HTTP_ORIGIN'] ?? '';
if (strpos($origin, 'chrome-extension://') === 0) {
    // Es una extensión de Chrome
    header('Access-Control-Allow-Origin: ' . $origin);
    header('Access-Control-Allow-Credentials: true');
} elseif ($origin === 'https://0ln.eu' || $origin === 'https://www.0ln.eu') {
    // Es tu dominio
    header('Access-Control-Allow-Origin: ' . $origin);
    header('Access-Control-Allow-Credentials: true');
} else {
    // Para otros orígenes, no permitir credentials
...
file_put_contents('delete_debug.log', json_encode($debug) . "\n", FILE_APPEND);
